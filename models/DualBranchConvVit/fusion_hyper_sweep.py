...
def run_search(base_config: dict, fusion_type: str, phase_num: int) -> list[dict]:
    ...
    print("\n" + "-" * 60)
    print(
        f"Phase {phase_num} search ({fusion_type.upper()}): "
        f"{n_total} configs x {SEARCH_EPOCHS} epochs"
    )
    ...
    print(f"\n[Phase {phase_num} {fusion_type}] Full ranking saved to: {csv_path}")

    print(f"\n[Phase {phase_num} {fusion_type}] Top-{SEARCH_TOP_K} configs:")
    for rank, r in enumerate(results[:SEARCH_TOP_K], 1):
        marker = "  (send to Phase 3)" if rank <= FINAL_TOP_K else ""
        print(
...
def run_final_training(
    ...
    print("\n" + "-" * 60)
    print("Phase 3: final training (best config per fusion)")
    print(f"{total_runs} runs x {FINAL_EPOCHS} epochs")
    print("-" * 60 + "\n")
...
def print_comparison_table(results: list[dict]) -> None:
    ...
    print("\n" + "-" * len(header))
    print("Fusion comparison (Phase 3: best vs best)")
    print("-" * len(header))
    print(header)
    print(sep)
    ...
    if len(best_per_fusion) == len(FUSION_TYPES):
        ...
        print(f"\nAttention - Concat: delta macro recall = {d_rec:+.4f} | delta viral recall = {d_viral:+.4f}")
        print(f"Winner by test macro recall: {winner.upper()}")
...
def save_results(final_results: list[dict]) -> None:
    ...
    print(f"[INFO] Phase-3 results saved to: {FINAL_CSV}")
    print(f"[INFO] Ranked results saved to: {RANKED_CSV}")
    ...
    for r in ranked:
        ...
        if rd and rd not in best_run_dirs:
            ...
            if leftover.exists():
                leftover.unlink()
    print(f"[INFO] Non-winning Phase 3 weights removed: kept {len(FUSION_TYPES)} .pth files")
...
def main() -> int:
    ...
    total_epoch_bound = (
        2 * n_search_configs * SEARCH_EPOCHS
        + len(FUSION_TYPES) * FINAL_TOP_K * FINAL_EPOCHS
    )

    print(f"[INFO] DualBranchConvViT fusion sweep started at {datetime.now().isoformat()}")
    print(f"[INFO] Phase 1: search CONCAT     - {n_search_configs} configs x {SEARCH_EPOCHS} epochs")
    print(f"[INFO] Phase 2: search ATTENTION  - {n_search_configs} configs x {SEARCH_EPOCHS} epochs")
    print(f"[INFO] Phase 3: train best {FINAL_TOP_K}/fusion x {len(FUSION_TYPES)} fusions x {FINAL_EPOCHS} epochs")
    ...
    print(f"\n[INFO] Fusion sweep complete. Results under: {SWEEP_DIR.resolve()}")
    return 0
