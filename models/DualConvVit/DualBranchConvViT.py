...
def main():
    ...
    official_val_df = df[df["split"] == "val"].copy()
    ...
    # The official validation split is far too small to select models on, so we
    # carve a stratified internal validation fold out of TRAIN for checkpoint
    # selection and hyperparameter ranking. The official TEST split is left
    # untouched and remains the headline reported metric; the official (tiny)
    # val split is reported for transparency only and never used for selection.
    ...
    print(f"[INFO] Splits -> Train(fit): {len(train_df)} | "
          f"Internal-Val: {len(val_df)} (stratified from train) | "
          f"Test: {len(test_df)} | Official-Val: {len(official_val_df)}")
    ...
    official_val_loader = DataLoader(PreprocessedChestXrayDataset(official_val_df, transform=transform), batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
    ...
    for fusion_strategy in fusion_types:
        ...
        criterion = nn.CrossEntropyLoss(weight=class_weight, label_smoothing=dl_cfg.get("label_smoothing", 0.1))
        optimizer = torch.optim.AdamW(head_params, lr=dl_cfg.get("learning_rate", 1e-4), weight_decay=dl_cfg.get("weight_decay", 1e-4))
        ...
        for epoch in range(epochs):
            if epoch == freeze_epochs and freeze_epochs > 0:
                ...
                main_lr = dl_cfg.get("learning_rate", 1e-4)
        ...
        # TTA matches the ViT baseline (use_tta: true). It runs only on
        # reportable runs: standalone runs (env unset) and the sweep's final
        # Phase 3 runs (DUAL_BRANCH_SAVE_WEIGHTS="1"). The cheap search runs
        # (="0") skip it, since they are ranked on the val fold, not test.
        save_weights_env = os.environ.get("DUAL_BRANCH_SAVE_WEIGHTS")
        is_reportable = save_weights_env != "0"
        use_tta = bool(dl_cfg.get("use_tta", True)) and is_reportable
        ...
        # Transparency only: report the official (tiny) val split. NOT used for
        # model selection or hyperparameter ranking.
        off_preds, off_labels = [], []
        with torch.no_grad():
            for images, labels in official_val_loader:
                images, labels = images.to(device), labels.to(device)
                if use_tta:
                    preds = torch.max(tta_predict(model, images, use_amp), 1)[1]
                else:
                    preds = torch.max(model(images), 1)[1]
                off_preds.extend(preds.cpu().tolist())
                off_labels.extend(labels.cpu().tolist())
        if off_labels:
            off_metrics = evaluator.compute_metrics(off_labels, off_preds)
            print(f"[INFO] Official val ({len(off_labels)} imgs, not used for selection) -> "
                  f"macro recall={off_metrics['macro_recall']:.4f} | "
                  f"macro f1={off_metrics['macro_f1']:.4f}")
